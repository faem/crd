...
type CustomPodLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.CustomPod, err error)
...
// List lists all CustomPods in the indexer.
func (s *customPodLister) List(selector labels.Selector) (ret []*v1alpha1.CustomPod, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.CustomPod))
	})
	return ret, err
}
...
type CustomPodNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.CustomPod, error)
...
// List lists all CustomPods in the indexer for a given namespace.
func (s customPodNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.CustomPod, err error) {
...
// Get retrieves the CustomPod from the indexer for a given namespace and name.
func (s customPodNamespaceLister) Get(name string) (*v1alpha1.CustomPod, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("custompod"), name)
	}
	return obj.(*v1alpha1.CustomPod), nil
}
